...
def save_class_metrics(all_classes, all_predictions, all_labels, filename):
    # Convert to class indices if they're not already
    ...
    precision, recall, f1, support = precision_recall_fscore_support(labels, predictions, zero_division=0)
    ...
    with open(filename, 'w', newline='') as f:
        ...
        for i, cls in enumerate(all_classes):
            writer.writerow([cls, precision[i], recall[i], f1[i], support[i]])
...
def evaluate_protonet(model, test_sampler, reverse_class_map=None):
    ...
    with torch.no_grad():
        ...
        for support_images, support_labels, query_images, query_labels, classes in progress_bar:
            ...
            # Map back to actual class names for per-class metrics
            if reverse_class_map:
                for cls in classes:
                    all_raw_classes.append(cls)

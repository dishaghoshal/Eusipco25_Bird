...
def compute_distance(query_feature, prototype, use_cosine=True):
    """Compute distance between query feature and prototype using specified metric"""
    if use_cosine:
        ...
        similarity = torch.matmul(query_feature_norm, prototype_norm.transpose(0, 1))
        
        # Convert to distance (1 - similarity)
        return 1 - similarity
    else:
        # Euclidean distance
        return F.pairwise_distance(query_feature, prototype.unsqueeze(0))

def predict_sample(model, sample_path, prototypes, use_cosine=True):
    ...
    with torch.no_grad():
        ...
        for class_name, prototype in prototypes.items():
            distance = compute_distance(query_feature, prototype, use_cosine).item()
...
def test_all_samples(model, test_dir, prototypes, classes, use_cosine=True):
    ...
    for file_name in tqdm(all_files, desc="Testing samples"):
        ...
        predicted_species, confidence, sorted_confidences = predict_sample(
            model, file_path, prototypes, use_cosine)
...
def analyze_confidence_vs_accuracy(results):
    ...
    confidence_levels = [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.0]
    threshold_results = []
...
def main():
    parser = argparse.ArgumentParser(description='Test contrastive ProtoNet on all bird audio samples')
    parser.add_argument('--model_path', type=str, default='best_protonet.pth', help='Path to saved model')
    ...
    parser.add_argument('--use_cosine', action='store_true', help='Use cosine distance instead of Euclidean')
    
    args = parser.parse_args()
    
    print(f"Testing with {'cosine' if args.use_cosine else 'Euclidean'} distance metric")
    ...
    results = test_all_samples(model, test_dir, prototypes, classes, args.use_cosine)
    ...
    for r in threshold_results:
        print(f"{r['threshold']:.1f}      | {r['accuracy']:.2f}%   | {r['coverage']:.2f}%  | {r['samples']}/{total}")
    ...
    # Create feature visualization if applicable
    if len(results) > 0:
        # Find the most problematic species pairs
        if len(sorted_confusion) > 0:
            most_confused_pair = sorted_confusion[0][0]
            true_species, pred_species = most_confused_pair
            
            print(f"\nMost confused species pair: {true_species} predicted as {pred_species}")
            print("This may indicate that these species have similar vocalizations.")
    
    print("\nAnalysis complete! Check the generated CSV files and plots for detailed results.")
    print("\nTip: Use the contrastive model for better species separation.")
